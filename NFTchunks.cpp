...
void splitFile(const std::string& inputFileName, const std::string& outputPrefix, size_t chunkSize) {
    ...
    while (!inputFile.eof()) {
        ...
        if (bytesRead > 0) {
            std::string outputFileName = outputPrefix + std::to_string(fileCounter++);
...
void mergeChunks(const std::string& inputPrefix, const std::string& outputFileName) {
    ...
    do {
        inputFileName = inputPrefix + std::to_string(chunkCounter++);
        std::ifstream inputFile(inputFileName, std::ios::binary);
...
int main() {
    ...
    std::string outputPrefix = "output_chunk_";
    size_t chunkSize = 1024; // Set your desired chunk size

    splitFile(inputFileName, outputPrefix, chunkSize);

    std::string inputPrefix = "output_chunk_";
